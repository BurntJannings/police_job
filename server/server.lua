...
RegisterNetEvent("lawmen:jailbreak")
...
RegisterServerEvent("lawmen:taketime")
...
print('it worked')
        if result[1] ~= nil then
            ...
            local newtime = 25
            local identifier = result[1]["identifier"]
            exports.ghmattimysql:execute("UPDATE jail SET time = @time WHERE identifier = @identifier", {["@time"] = time - newtime, ["@identifier"] = identifier})
        end
    end)
end)
